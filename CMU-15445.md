...
本项目采用快照隔离的方式进行处理，与一般的快照隔离不同的是本项目快照隔离采用事务ID + 时间戳的方式，元组处于修改状态时事务标记元组的时间戳为事务ID
...
（1）预写日志WAL：**WAL（Write-Ahead Logging，预写日志）**是数据库管理系统实现事务持久性与故障恢复的核心机制，其核心原则是**“先记录日志，后修改数据”**，即所有数据修改操作在提交前必须先写入日志文件，再应用到实际数据文件。这一机制确保了事务的原子性和数据一致性，即使系统崩溃也能通过重放日志恢复数据
