...
auto UpdateExecutor::Next(Tuple *tuple, RID *rid) -> bool {
  ...
  while (child_executor_->Next(&child_tp, &child_rd)) {
    ...
    if (old_meta.ts_ == txn->GetTransactionTempTs()) {
      ...
      new_undolink.is_inprogress_ = old_undolink_opt->is_inprogress_;
    ...
    // 原子更新操作 先锁住 看旧数据 再更新
    auto update_state = table_info_->table_->UpdateTupleInPlace(
        new_tpmeta, new_tuple, child_rd, [txn](const TupleMeta &meta, const Tuple &table, RID rid) -> bool {
    ...
    // 处理主键索引的更新 必须先存到一个地方再统一删除更新
    if (exist_prim_key) {
      tuples_.emplace_back(new_tuple, old_tuple, child_rd);
    }
  ...
  // 此时再统一进行事务中的主键索引更新
  if (update_pm_key_) {
    UpdatePrmIndexs(table_info_, indexs_info_[0], tuples_, txn);
  }
