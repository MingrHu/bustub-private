...
HashJoinExecutor::HashJoinExecutor(ExecutorContext *exec_ctx, const HashJoinPlanNode *plan,
    ...
    plan_(plan),left_child_(std::move(left_child)),right_child_(std::move(right_child)),
    left_key() {}
...
void HashJoinExecutor::Init() { 
  ...
  right_child_->Init();
  hash_table_.clear();
  next_left_tuple = true;
  inner_indx = 0;
  left_key.keys_.clear();
...
auto HashJoinExecutor::Next(Tuple *tuple, RID *rid) -> bool { 
  ...
  while(!next_left_tuple || left_child_->Next(&left_tuple_,&left_rid_)){
    ...
    if(next_left_tuple){
      ...
      left_key.keys_ = org_key;
    }
    next_left_tuple = false;
    // 先看是否有匹配的
    if(hash_table_.count(left_key) != 0){
      for(size_t idx = inner_indx;idx < hash_table_[left_key].size();idx++){
        ...
        for(size_t i = 0;i < plan_->GetRightPlan()->OutputSchema().GetColumnCount();i++){
          values.emplace_back(hash_table_[left_key][idx].GetValue(
            &plan_->GetRightPlan()->OutputSchema(), i));
        }
    ...
    next_left_tuple = true;
    // 如果没有任何匹配的且为左连接
    if(inner_indx == 0 && plan_->join_type_ == JoinType::LEFT){
      for(size_t i = 0;i < plan_->GetLeftPlan()->OutputSchema().GetColumnCount();i++){
      ...
      for(size_t i = 0;i < right_child_->GetOutputSchema().GetColumnCount();i++){
        // 添加NULL值
        values.emplace_back(ValueFactory::GetNullValueByType(plan_->GetRightPlan()->
        OutputSchema().GetColumn(i).GetType()));
      }
      *tuple = {values,&plan_->OutputSchema()};
      *rid = {};
      return true;
